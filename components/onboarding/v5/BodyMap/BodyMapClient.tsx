...
import { ArrowLeft, Activity, Sparkles, Brain, User, Heart, Smile, Wind, Scale, Baby } from 'lucide-react';
import { MainCategory, BodyPart } from '@/lib/onboarding/v5/types';

interface CategoryData {
    id: string; // was MainCategory, now generic string from DB
    name_en: string;
    name_ar: string;
    description_en?: string;
    icon?: string;
    // Map to old props for compatibility if needed, or update UI
}
...
export default function BodyMapClient() {
    ...
    // Helper to get icon for body part
    const getBodyPartIcon = (partName: string) => {
        const name = partName.toLowerCase();
        if (name.includes('face')) return Smile;
        if (name.includes('hair')) return Wind;
        if (name.includes('chest')) return Heart;
        if (name.includes('abdomen') || name.includes('tummy')) return Scale;
        if (name.includes('brain') || name.includes('mind')) return Brain;
        if (name.includes('reproductive') || name.includes('fertility')) return Baby;
        return Activity;
    };
        ...
        return (
            <div className="flex flex-col items-center justify-center w-full max-w-5xl mx-auto p-6 min-h-[80vh]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
                    {categories.map((cat) => {
                        // Use default icons if dynamic mapping fails
                        const Icon = getCategoryIcon(cat.icon || 'Activity');
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="bg-teal-50 p-6 rounded-full mb-6 group-hover:scale-110 transition-transform duration-300 group-hover:bg-teal-100 relative z-10">
        ...
        <div className="flex flex-col items-center w-full max-w-4xl mx-auto p-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {(isLoading || (isDataLoading && bodyParts.length === 0)) ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full">
                    {bodyParts.map((part) => {
                        const Icon = getBodyPartIcon(part.nameEn);
                        return (
                            <button
                                key={part.id}
                            ...
                            >
                                <div className="p-4 bg-gray-50 rounded-full mb-4 group-hover:bg-teal-50 transition-colors">
                                    <Icon className="w-8 h-8 text-gray-600 group-hover:text-teal-600 transition-colors" />
                                </div>
