-- ==============================================================================
-- CLEAN AND SEED SCRIPT (v4 - FIXED COLUMNS)
-- 1. Removed non-existent tables from TRUNCATE blacklist.
-- 2. Removed non-existent columns (gender, phone_number) from profiles INSERT.
...
BEGIN
    ...
    -- TRUNCATE TABLE medical_records CASCADE; -- Does not exist
    -- TRUNCATE TABLE form_responses CASCADE;  -- Check if exists
    ...
    IF doctor_auth_id IS NOT NULL THEN
        -- Removed 'gender', 'phone_number', 'city' if not sure. Keeping city for now as it's common.
        -- If city fails, we will remove it too.
        INSERT INTO profiles (id, full_name_ar, full_name_en, role, city)
        VALUES (
            doctor_auth_id,
            'د. مها',
            'Dr. Maha',
